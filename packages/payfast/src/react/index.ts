...
export function useTransactions(
  ...
  const component = isComponentApi(componentOrFilters)
    ? componentOrFilters
    : usePayfast();
  const actual = isComponentApi(componentOrFilters) ? filters : componentOrFilters;
...
export function useSubscription(
  ...
  const component = isComponentApi(componentOrToken)
    ? componentOrToken
    : usePayfast();
  const actualToken = isComponentApi(componentOrToken) ? token! : componentOrToken;
...
export function useAdhocCharge(
  componentOrUndefined?: ComponentApi,
): {
  charge: (args: {
    token: string;
    amount: number;
  ...
  const charge = useCallback(
    async (args: {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Ad-hoc charge failed",
        );
        throw err;
      } finally {
        setLoading(false);
...
export function usePayfastOnsite(
  componentOrOpts: ComponentApi | (CheckoutOpts & { returnUrl: string; cancelUrl: string }),
  opts?: CheckoutOpts & { returnUrl: string; cancelUrl: string },
): {
  generateOnsite: () => Promise<void>;
  paymentIdentifier: string | null;
  ...
  const component = isComponentApi(componentOrOpts)
    ? componentOrOpts
    : usePayfast();
  const onsiteOpts = isComponentApi(componentOrOpts)
    ? opts!
    : componentOrOpts;
